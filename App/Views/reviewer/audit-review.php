...
if (
    ...
    $gtMrk = generate_table_markup(
        $data,
        $data['data']['data_array'],
        $data['data']['filter_type']
    );
} else {
    $gtMrk = $data['noti']::cError(
        'Your current assessment has <b>0</b> compliance points. Are you sure you want to complete assessment?',
        'warning'
    );
...
if (
    array_key_exists('review_timeline_status', $data['data']) &&
...
if (is_array($timeline_status_array) && $compliancePointCount) {
    ...
    $mrk_str .= FormElements::generateFormStart([
    ...
    $mrk_str .= FormElements::generateSubmitButton('', [
        'name'  => 'observationActionSubmit',
        'value' => 'Save'
    ]);
...
echo '<div class="card-header text-uppercase">Review ' .
     ($data['data']['assesmentData']->audit_status_id == 2 ? 'Audit' : 'Compliance') .
     '</div>' . "\n";
...
?>
<div id="auditMeta"
     data-audit-status="<?= $data['data']['assesmentData']->audit_status_id ?>">
</div>
