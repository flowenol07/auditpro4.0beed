        ...
        echo '<div class="table-repsonsive">' . "\n";
            echo '<table class="table table-bordered">' . "\n";
                echo '<tr>
                    ...
                    if($data['data']['assesmentData'] -> audit_status_id == 5)
                    {
                ...
                foreach($data['data']['observation'] as $cObv => $cObvDetails)
                {
                    echo '<tr>' . "\n";
                        ...
                        if($data['data']['assesmentData'] -> audit_status_id == 5)
                        {
                            echo (isset(AUDIT_STATUS_ARRAY['compliance_review_action'][4]) ? ('<td align="center">'. $cObvDetails['onhold'] .'</td>') : '');
                            echo (isset(AUDIT_STATUS_ARRAY['compliance_review_action'][5]) ? ('<td align="center">'. $cObvDetails['cf_point'] .'</td>') : '');
                        }
