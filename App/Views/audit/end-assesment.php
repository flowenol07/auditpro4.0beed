    ...
    is_array($data['data']['db_sets']) && sizeof($data['data']['db_sets']) > 0 )
{
    ...
    foreach( $data['data']['db_menu'] as $cMenuId => $cMenuDetails ) {
        ...
        {
            // function call
            $cfFuncCall = audit_end_asses_generate_cf_markup($data, $data['data']['db_assesment_data']);
            $complianceCnt += $cfFuncCall['cnt'];
            // $pendingCnt += $cfFuncCall['pending']; // only for reassesment
                        ...
                        for($i = 0; $i < $cSetLoop; $i++) {
                            ...
                            foreach($cCatSets as $cSetId):
                                ...
                                {    
                                    ...
                                    if($responseData['compliance'] != null) $complianceCnt += $responseData['compliance'];
                                }
    ...
    echo '<div class="mb-3">
        <button id="showPending" class="btn btn-primary">Show Pending</button>
    ...
    <p class="text-danger mb-2">Pending Assesment Points: <span id="pendingPoints" class="font-medium"><?= $pendingCnt ?></span></p>
    <?= $data['noti']::cError('<b>Note </b> - Total Compliance Points: <span id="compliancePoints" class="font-medium">'. $complianceCnt .'</span>', 'warning'); ?>
