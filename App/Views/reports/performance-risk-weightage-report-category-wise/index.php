    ...
    echo '<div id="printContainer">' . "\n";
        ...
        foreach($data['data']['has_data'] as $cRiskId => $cRiskData):
            ...
            foreach(['general', 'advances', 'deposits'] as $cGenKey):
                ...
                if(isset($data['data']['data_array'][ $cGenKey ][ $cRiskId ]))
                {
                    $cRiskData = $data['data']['data_array'][ $cGenKey ][ $cRiskId ];
                    ...
                    $resMarkup .= '<tr>';
                        ...
                        $resMarkup .= '<td>'. string_operations($cGenKey, 'upper') .'</td>';
        ...
        $resMarkup .= '<table id="exportToExcelTable" class="table table-bordered v-table">
            <thead>
                <tr>
                    ...
                    <td colspan="7"><span class="font-medium">Number of Audits Conducted:</span> '. sizeof($data['data']['data_array']['no_of_asses']) .'</td>
                </tr>

                <tr>
                    <td colspan="6"><span class="font-medium">Number of Accounts (Deposits):</span> '. $data['data']['data_array']['total_deposits'] .'</td>
                    <td colspan="7"><span class="font-medium">Number of Accounts (Advances):</span> '. $data['data']['data_array']['total_advances'] .'</td>
                </tr>
                ...
                <tr>
                    ...
                    <td colspan="7"><span class="font-medium">Number of Accounts - Sampled (Advances):</span> '. $data['data']['data_array']['total_advances_sampling'] .'</td>
                </tr>';

        if( $data['request'] -> has('selectSearchTypeFilter') && 
            in_array($data['request'] -> input('selectSearchTypeFilter'), [3,4]))
            $resMarkup .= '<tr><td colspan="13"><span class="font-medium">Assesment Period:</span> '. $data['data']['data_array']['no_of_asses'][ array_keys($data['data']['data_array']['no_of_asses'])[0] ] .'</td></tr>';
        elseif( $data['request'] -> has('selectSearchTypeFilter') && 
                in_array($data['request'] -> input('selectSearchTypeFilter'), [5,6]))
            $resMarkup .= '<tr><td colspan="13"><span class="font-medium">Period:</span> '. $data['request'] -> input('startDate') . ' - ' . $data['request'] -> input('endDate') .'</td></tr>';
        ...
        $resMarkup .= '<tr>
            ...
            <td align="right" class="font-bold"></td>
