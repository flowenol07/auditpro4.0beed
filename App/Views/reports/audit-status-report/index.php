                    ...
                    foreach($data['data']['details_of_audit_data'] as $cKey => $cAssesData)
                    {
                        ...
                        '<tr>
                            ...
                            <td style="width:10%" class="text-center">';
                                ...
                                {
                                    // pending and reaudit
                                    if($cAssesData -> is_limit_blocked == 1)
                                        echo "BLOCKED";

                                    elseif(($cAssesData -> audit_due_date >= date("Y-m-d")) || ($cAssesData -> audit_end_date != ''))
                                        echo $data['data']['auditStatusArray'][$cAssesData -> audit_status_id]['status'];

                                    elseif(in_array( $cAssesData -> audit_status_id, [ ASSESMENT_TIMELINE_ARRAY[1]['status_id'], ASSESMENT_TIMELINE_ARRAY[3]['status_id']] ) && strtotime($cAssesData -> audit_due_date) < strtotime(date($GLOBALS['dateSupportArray'][1])))
                                        echo $data['data']['auditStatusArray'][$cAssesData -> audit_status_id]['status'] . "<br><span style='color:red; font-size:14px'>( EXPIRED ON <br>" . $cAssesData -> audit_due_date . ' )</span>';
                                }
                                else
                                {
                                    $auditStatus = true;
                            ...
                            if($auditStatus)
                            {
                                ...
                                if(in_array($cAssesData -> audit_status_id, [4, 5, 6])) 
                                {
                                    ...
                                    elseif($cAssesData -> compliance_due_date >= date("Y-m-d"))
                                        echo $data['data']['complianceStatusArray'][$cAssesData -> audit_status_id]['status'];

                                    elseif(in_array( $cAssesData -> audit_status_id, [ ASSESMENT_TIMELINE_ARRAY[4]['status_id'], ASSESMENT_TIMELINE_ARRAY[6]['status_id']] ) && strtotime($cAssesData -> compliance_due_date) < strtotime(date($GLOBALS['dateSupportArray'][1])))
                                        echo $data['data']['complianceStatusArray'][$cAssesData -> audit_status_id]['status'] . "<br><span style='color:red; font-size:14px'>( EXPIRED ON <br>" . $cAssesData -> compliance_due_date . ' )</span>';
                                }
                                else
                                    echo "COMPLETED";
