...
<div class="row">
    ...
    <div class="col-md-4">
        <?php
            //comp_status	
            $markup = FormElements::generateLabel('comp_status', 'Select Compliance Status');

            if(is_array($data['data']['complianceStatusArray']) && sizeof($data['data']['complianceStatusArray']) > 0)
            {
                $markup .= FormElements::generateSelect([
                    "id" => "comp_status", "name" => "comp_status",
                    "options" => $data['data']['complianceStatusArray'],
                    "options_db" => ["type" => "arr", "val" => "status"],
                    "selected" => isset($data['data']['auditAssesData'][0] -> audit_status_id) ? ($data['request'] -> input('comp_status', $data['data']['auditAssesData'][0] -> audit_status_id)) : $data['request'] -> input('comp_status'),
                ]);
            }
            else
                $markup .= $data['noti']::getCustomAlertNoti('noDataFound');

            echo FormElements::generateFormGroup($markup, $data, 'comp_status');
        ?>
    </div>
</div>
