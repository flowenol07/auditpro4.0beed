...
<div class="row">
        ...
        <?php
            ...
            if(is_array($data['data']['db_year_data']) && sizeof($data['data']['db_year_data']) > 0)
            {
                $markup .= FormElements::generateSelect([
                    ...
                    "selected" => isset($data['data']['target_data'][0] -> year_id) ? ($data['request'] -> input('financial_year', $data['data']['target_data'][0] -> year_id)) : ($data['request'] -> input('financial_year')),
                ]);
            }
            else
    ...
    <?php 
    if(!empty($data['data']['target_data'])) { ?>
    <div class="col-md-6">
        <?php
            //branch	
